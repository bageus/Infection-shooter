...
## Progress
Числовые поля палитры принимают клавиатурный ввод при фокусе внутреннего LineEdit, а горячие клавиши планировщика в этот момент не перехватывают цифры. Коллизия обычной и аварийной двери сохраняет отверстие в стене: неподвижная часть использует форму меша, полотно блокирует проход закрытым и освобождает его при открытии.

Исправлена регрессия палитры: события мыши на панели передаются GUI, но не вызывают размещение на поле. При разрушении столешницы оставшиеся ножки снимаются с `freeze` и получают боковой и вращательный импульс, поэтому падают отдельно. Предупреждения о затенении имён переменных и двух неиспользованных параметрах в `staged_table.gd` устранены.
...
Изолированный Godot 4.5.2 подтвердил доставку клика в ItemList без размещения объекта, опрокидывание всех четырёх ножек после разрушения центра, прежние адресные попадания, падение стола после второй ножки и лимит 40 декалей.
...
В полной сцене Godot 4.7.2 визуально проверить выбор палитры и падение всех четырёх ножек после выстрела в центр столешницы.

...
- размер GDScript-файлов и функций без механического дробления логики.
...
## Ограничения размера исходного кода

- Производственный GDScript: предупреждение после 300 строк, ошибка после 600.
- Тестовый GDScript: предупреждение после 500 строк, ошибка после 900.
- Функция: предупреждение после 50 строк, ошибка после 100.
- Исключение требует ADR и явной записи в архитектурной политике.

Разделение выполняется по ответственности и владельцу состояния, а не ради количества файлов.
